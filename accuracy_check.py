...
clients = DocumentIntelligenceClient(endpoint=AZURE_ENDPOINT, credential=AzureKeyCredential(AZURE_KEY))
...
def extract_text_gemini(file_path: str) -> tuple[str, int]:
    ...
    prompt = "Extract all visible text from this document as plain text. Only text, nothing extra."
...
def extract_text_azure(file_path: str) -> tuple[str, int]:
    ...
    poller = clients.begin_analyze_document(
...
def word_set_match(extracted: str, manual: str):
    """Exact match but ignores order, counts matter."""
    return Counter(normalize_text(extracted).split()) == Counter(normalize_text(manual).split())
...
# ---------------- STREAMLIT UI ------------------
st.set_page_config(page_title="OCR Accuracy Evaluation", layout="wide")
...
if uploaded_file and manual_text:
    ...
    with st.spinner("Running Azure OCR..."):
        azure_text, _ = extract_text_azure(temp_path)
    ...
    with col3:
        st.markdown("### 🟨 Azure OCR")
        st.text_area("Extracted Text", azure_text, height=200)

    # --- Accuracy Metrics ---
    st.subheader("📌 Accuracy Metrics")

    for method, extracted in [
        ("Tesseract", tess_text),
        ("Gemini", gem_text),
        ("Azure", azure_text),
    ]:
        sem_score = semantic_similarity(extracted, manual_text)
        word_match_score = word_set_match(extracted, manual_text)
        word_err = wer(manual_text, extracted)
        char_err = cer(manual_text, extracted)

        st.markdown(f"### {method} OCR")
        st.write(f"🔹 Semantic Similarity: **{sem_score:.2f}**")
        st.write(f"🔹 Word-level Exact Match: **{word_match_score}**")
        st.write(f"🔹 Word Error Rate (WER): **{word_err:.2%}**")
        st.write(f"🔹 Character Error Rate (CER): **{char_err:.2%}**")
